...
document.addEventListener("DOMContentLoaded", function () {
  ...
  arrowBtns.forEach((btn) => {
    btn.addEventListener("click", () => {
      carousel.scrollLeft +=
        btn.id === "left-icon" ? -firstCardWidth : firstCardWidth;
    });
  });
});

const allSkeleton = document.querySelectorAll(".skeleton");
const allInnerSkeletonText = document.querySelectorAll(".inner-skeleton");
const toBeVisibleElements = document.querySelectorAll(".hide-before-load");
...
window.addEventListener("load", function () {
  setTimeout(() => {
    ...
    allInnerSkeletonText.forEach((item) => {
      item.classList.remove("inner-skeleton");
    });

    toBeVisibleElements.forEach((item) => {
      item.classList.remove("hide-before-load");
    });
  }, 2000);
});
